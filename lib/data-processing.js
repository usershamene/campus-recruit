...
// ── 反爬/聚合站域名黑名单：applyUrl/announcementUrl 命中即视为无效投递入口 ──
// 维护约定：发现源网站把官方链接换成聚合站跳转时，把该域名加到这里（子域名一并命中）
const ANTI_CRAWL_DOMAINS = [
  'givemeoc.com',   // OC 聚合站：求职方舟部分岗位的投递链接被替换为该站跳转
];

function isAntiCrawlUrl(url) {
  if (!url) return false;
  const m = String(url).match(/^https?:\/\/([^\/?#]+)/i);
  if (!m) return false;
  const host = m[1].toLowerCase().replace(/^www\./, '');
  return ANTI_CRAWL_DOMAINS.some(d => host === d || host.endsWith('.' + d));
}
...
// ── 国企白名单：人工核定的真国企（启发式规则无法覆盖时的精确兜底）──
// 维护约定：发现"漏识别"的真国企，把其名称中的特指片段加到这里（用 includes 匹配，片段要足够独特）
const SOE_WHITELIST = [
  '扬农化工',        // 中化集团旗下
  '汕头超声印制板',  // 超声电子（国资）旗下
  '上海光通信',      // 上海仪电集团旗下
  ...
  '青少年训练',      // 事业单位（上海）
  '贵州磷化',        // 贵州磷化集团（省属国企）
  '规划国土发展',    // 深圳规划国土发展研究中心（事业单位）
  '山东天气',        // 山东省气象局
  '格兰云天',        // 深圳国资酒店集团（名字无特征）
  '宇石空间探索',    // 航天背景（民企标注缺失时兜底）
  '华中长江光电',    // 兵装/中船系
  '金融大数据',      // 北京国资
  '世纪人才',        // 人才服务中心系
  '北京脑所',        // 北京脑科学与类脑研究所（新型研发机构）
];

// ── 国企判定规则引擎 ──
function isSOE(companyName, applyUrl, announceUrl) {
  if (!companyName) return false;
  // 规则0：人工核定白名单优先（覆盖排除表与启发式）
  if (SOE_WHITELIST.some(name => companyName.includes(name))) return true;
  ...
  // 规则2 已移除：公司名以行政区划开头 ≠ 国企（大量民企/外企以城市名开头，
  //   如"广州元游信息技术有限公司""上海沪工"）。真国企由规则4-7 的"地名 + 国企特征词"组合判定。
  ...
  const soeCityWords = [
    // 原有
    '城建','城投','投资控股','投资集团','交通','水利','水务集团','公用事业','国资','资产经营',
    // 金融（银行/农信体系几乎全为国资，个别民营银行见 SOE_EXCLUDE）
    '银行','农商行','农村商业','农村信用',
    // 传媒文卫（事业单位/国企）
    '广播电视台','日报','晚报','传媒','出版','博物馆','图书馆','医院','学校','学院','研究院','研究所','设计院','勘察设计',
    // 城投/国资平台延伸
    '城市发展','城市产业','城市更新','建设发展','建设投资','轨道建设','轨道交通','交发','文旅','旅游发展','文化投资',
    // 能源/产业平台
    '能源集团','能源投资','经济建设投资','机电控股','电气装备','实业发展','人才发展','人才集团','人才服务',
    // 央企品牌/特定平台
    '欢乐谷','金融发展','航空产业','化工集团','开发区','市政',
  ];
...
/**
 * 判定单条岗位的最终招聘类型（国企判定 + 类型规范化 + 推断）
 * 原地更新 job.recruitmentType 并返回；由 processData 与数据修复脚本共用，避免逻辑重复。
 * @param {object} job 岗位对象
 * @param {number} currentYear 当前年份（用于春招时间纠正）
 * @returns {string} 最终招聘类型
 */
function resolveRecruitmentType(job, currentYear) {
  ...
  if (rawType === '国企' || companyTypeVal === '国企' || companyTypeVal === '央国企' || companyTypeVal === '央企' || companyTypeVal === '事业单位') {
    job.recruitmentType = '央国企招聘';
    return job.recruitmentType;
  }
  // 1. 数据源明确标注为民企/外企/合资的，不再走 isSOE 猜测（避免启发式误判）
  const isPrivateLabeled = /民营|民企|外企|合资|外商/.test(companyTypeVal);
  // 2. 无明确标注的才走 isSOE() 兜底判定
  if (!isPrivateLabeled && isSOE(job.company, job.applyUrl, job.announcementUrl)) {
    job.recruitmentType = '央国企招聘';
    return job.recruitmentType;
  }
  // 2. TYPE_MAP 规范化
 ...
 * @returns {{processed: Array, loginFiltered: number, expired: number, antiCrawlRemoved: number}}
 */
function processData(jobs) {
  ...
  const cleaned = jobs.filter(job => {
    ...
    // 反爬链接过滤：命中黑名单域名的链接清空；清空后无任何投递/公告入口 → 移除岗位
    const hadAnyEntry = !!(job.applyUrl || job.announcementUrl);
    if (isAntiCrawlUrl(job.applyUrl)) job.applyUrl = '';
    if (isAntiCrawlUrl(job.announcementUrl)) job.announcementUrl = '';
  ...
  for (const job of cleaned) {
    resolveRecruitmentType(job, currentYear);
  }
  ...
  return { processed: filtered, loginFiltered: loginWallCount, expired: cleaned.length - filtered.length, antiCrawlRemoved };
}

module.exports = {
  TYPE_MAP, PROVINCES, CITIES, SOE_EXCLUDE, SOE_WHITELIST,
  deduplicate, isSOE, splitPositions, inferType, processData, resolveRecruitmentType, isAntiCrawlUrl, ANTI_CRAWL_DOMAINS,
};
