...
// ── 国企判定 ──
const SOE_STRONG = [
  '国资','国有','国资委','国有资本','市属国企','区属国企','县属国企','省属国企',
...
const SOE_WEAK = [
  ...
  '公路','港口','码头','航运','海运','航天','航空','电科',
  '发电','核电','水电','火电','风电','光伏',
];
const SOE_EXCLUDE = [
  '教育','培训','学院','大学','中学','小学','幼儿园','教培','工作室','合伙','个体','民营','私人','外资','外商',
  '机器人','智能','半导体','芯片','集成电路','微电子','光电','新能源','新材料','环保','节能','碳','排放',
  '基金','私募','创投','风投','VC','PE',
  '药','制药','生物','医学','临床','检验','检测',
...
function isSOE(companyName) {
  if (!companyName) return false;
  if (SOE_EXCLUDE.some(kw => companyName.includes(kw))) return false;
  const strong = SOE_STRONG.filter(kw => companyName.includes(kw));
  const weak = SOE_WEAK.filter(kw => companyName.includes(kw));
  const hasChinaPrefix = ['中国','中华','国家','中央'].some(p => companyName.includes(p));
  const hasInstitution = ['院','所','中心','银行','保险','证券','信托','电信','移动','联通','邮政','民航','气象','铁路','公路','航道','港口','码头','航运','海运','航天','航空','电科','发电','核电','水电','火电','风电','光伏'].some(s => companyName.includes(s));
  const hasGov = ['局','委','办','事业单位','行政机关','管委会'].some(s => companyName.includes(s));
  return strong.length >= 1 ||
    (hasChinaPrefix && (hasInstitution || hasGov)) ||
    weak.length >= 2;
}
